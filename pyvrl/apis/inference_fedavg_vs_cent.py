...
def _load_checkpoint(_model, _chk_path):
    ...
    # here the model will only return the params
    return params

def _load_state_model(_chk_path):

    checkpoint = torch.load(_chk_path)
...
@torch.no_grad()
def test_clip_retrieval_fedvscent(model,
                        train_dataset,
                        test_dataset,
                        cfg,
                        args,
    ...
    def process_single_chkpt(_model, _train_dataset, _test_dataset, _cfg, _progress=False):
    ...
    if (len(checkpoint)<2) and (len(checkpoint>2)):
        raise ValueError("Checkpoint list should contain only two files")
    
    #######################################################################################################################################
    # This chunk of code is taken from: https://github.com/keskarnitish/large-batch-training/blob/master/PyTorch/plot_parametric_pytorch.py  
    ...
    if checkpoint[0] is not None and checkpoint[0].endswith('.npz'):
        m1_params = _load_checkpoint(model, checkpoint[0])
        # params_dict = zip(model.state_dict().keys(), m1_params)
        # m1 = OrderedDict({k: torch.from_numpy(v) for k, v in params_dict})
        
        logger.info("The first checkpoint.npz is being loaded...")
    else:
        logger.info("The first checkpoint.pth is being loaded...")
        m1 = _load_state_model(checkpoint[0])
        # m1 = m1['state_dict']
        

    if checkpoint[1] is not None and checkpoint[1].endswith('.npz'):
        m2_params = _load_checkpoint(model, checkpoint[1])
        # params_dict = zip(model.state_dict().keys(), m2_params)
        # m2 = OrderedDict({k: torch.from_numpy(v) for k, v in params_dict})
        logger.info("The second checkpoint.npz is being loaded...")

    else:
        logger.info("The second checkpoint.pth is being loaded...")
        m2 =_load_state_model(checkpoint[1])
        # m2 = m2['state_dict']
    
    base_name = os.path.basename(checkpoint[0])[:-4] if checkpoint[0] is not None else 'none'
    txt_path = os.path.join(cfg.work_dir,
                                test_dataset.name,
                                'clip_retrieval_acc_{}.txt'.format(base_name))
    
    mkdir_or_exist(os.path.dirname(txt_path))
    ########################################################################################################################################
    # This chunk of code is taken from: https://github.com/keskarnitish/large-batch-training/blob/master/PyTorch/plot_parametric_pytorch.py  
    
    params_dict = zip(m1.keys(), m2_params)
    m2 = OrderedDict({k: torch.from_numpy(v) for k, v in params_dict})
    for alpha in alpha_range: 
        mydict = {}
        for key, value in m1.items():
            mydict[key] = m1[key] * alpha + (1 - alpha) * m2[key]

        # state_dict = OrderedDict({k: v for k, v in mydict.items()})

        # model.load_state_dict(state_dict, strict=False)
        load_state_dict(model, mydict, strict=False)
        
        dataset_results = process_single_chkpt(model, train_dataset, test_dataset, cfg, progress)
        ...
        logger.info('-----------------------')
        logger.info('chk: {}'.format(checkpoint[0]))
        for k in dataset_results.keys():
            if k.startswith('top'):
