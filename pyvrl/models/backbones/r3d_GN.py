...
# This script file is for replacing the batch-norm with the group norm
...
def build_3d_conv(block_type,
    ...
    if block_type == '2.5d':
        ...
        _dict['conv_s'] = nn.Conv3d(
            ...
            bias=not with_bn)
        if with_bn:
            _dict['bn_s'] = nn.GroupNorm(2,mid_channels, eps=1e-3)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = build_3d_conv(block_type=block_type,
                                   ...
                                   with_bn=with_bn)
        if self.with_bn:
            self.bn1 = nn.GroupNorm(2,out_channels, eps=1e-3)
        self.conv2 = build_3d_conv(block_type=block_type,
                                   ...
                                   with_bn=with_bn)
        if self.with_bn:
            self.bn2 = nn.GroupNorm(2, out_channels, eps=1e-3)
        if down_sampling or in_channels != out_channels:
            self.downsample = build_3d_conv(block_type='3d',
                                            ...
                                            with_bn=with_bn)
            if self.with_bn:
                self.downsample_bn = nn.GroupNorm(2,out_channels, eps=1e-3)
        else:
    ...
    def build_stem_block(stem_type: str,
        ...
        if stem_type == '2.5d':
            ...
            _dict['conv'] = nn.Conv3d(
                ...
                bias=not with_bn)
            if with_bn:
                _dict['bn'] = nn.GroupNorm(2, 64, eps=1e-3)
...
@BACKBONES.register_module()
class R3D_GN(BaseResNet3D):
    def __init__(self, *args, **kwargs):
        super(R3D_GN, self).__init__(block_type='3d', *args, **kwargs)
