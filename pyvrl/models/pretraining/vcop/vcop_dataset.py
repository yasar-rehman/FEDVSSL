...
# This portion of code will be used to map the dataset classes to the registry as a string so it can be called easily
...
class VCOPDataset(Dataset):
    ...
    def evaluate(self, results, logger=None):
        if isinstance(results,list):
            # print(results)
            acc_dict = dict()
            acc_dict[f'mean_class_acc'] = sum(results)/len(results)
            return acc_dict
